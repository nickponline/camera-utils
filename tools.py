...
class Projector(object):
    ...
    def to_image(self, point):
        ...
        return self.sensor.project(self.pose, point)
...
class Camera(object):
    ...
    def c2i(self, x):
        xp = self.K[0, 2] + self.K[0, 0] * x[0]
        yp = self.K[1, 2] + self.K[1, 1] * x[1]
        return (xp, yp)
...
class PinholeCamera(Camera):
    ...
    def project(self, pose, point):
        p = np.matrix([point[0], point[1], point[2], 1]).T
        x = pose * p
        x = (x[0] / x[2], x[1] / x[2])
        return self.c2i(self.distort(x))
...
class BrownCamera(PinholeCamera):
    ...
    def distort(self, x):
        ...
        return (x[0] * coeff + self.p1 * xy * 2.0 + self.p2 * (r2 + x2 * 2.0),
                x[1] * coeff + self.p1 * (r2 + y2 * 2.0) + self.p2 * xy * 2.0)
